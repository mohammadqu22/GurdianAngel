#!/usr/bin/env python3
"""
Generates English TTS audio using Gemini 3.1 Flash TTS (paid tier).
Overwrites existing Google Cloud WaveNet English WAVs.

Run: python3 generate_english_gemini.py
"""
...
def main() -> None:
    ...
    for eid in EMERGENCY_IDS:
        json_path = os.path.join(SCRIPT_DIR, "assets", "data", f"{eid}.json")
        ...
        for step in data.get("steps", []):
            ...
            path = os.path.join(SCRIPT_DIR, "assets", "audio", "en", eid, f"step_{n}.wav")

            if os.path.exists(path):
                print(f"  [SKIP] en/{eid}/step_{n}.wav already exists")
                continue

            print(f"  [GEN]  en/{eid}/step_{n}.wav ...", end=" ", flush=True)
            try:
