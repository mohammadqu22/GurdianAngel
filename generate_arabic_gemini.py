#!/usr/bin/env python3
"""
Regenerates Arabic TTS audio using Gemini 3.1 Flash TTS.
Overwrites the existing Google Cloud WaveNet Arabic WAVs.

Run: python3 generate_arabic_gemini.py
"""
...
def main() -> None:
    ...
    for eid in EMERGENCY_IDS:
        json_path = os.path.join(SCRIPT_DIR, "assets", "data", "ar", f"{eid}.json")
        ...
        for step in data.get("steps", []):
            ...
            path = os.path.join(SCRIPT_DIR, "assets", "audio", "ar", eid, f"step_{n}.wav")

            if os.path.exists(path):
                print(f"  [SKIP] ar/{eid}/step_{n}.wav already exists")
                continue

            print(f"  [GEN]  ar/{eid}/step_{n}.wav ...", end=" ", flush=True)
            try:
